...
plotExon <- function(getRegionObject, ind=NULL, exonname=NULL, tstats, pos, annotation, counts, group, bppad = 50, axpad = 50, prettyskips = T, skiplines = T, countsheader=F, countssep="\t", tabname = NULL, plotfile = NULL, width = 900, height = 750, plottitle = NULL, chromosome,legendloc="bottomleft")
{
	if(!all(c("states.norle","states") %in% names(getRegionObject))) stop("getRegionObject must be a list with elements \"states.norle\" and \"states\" - usually the return of getRegions()")
	if(!all(c("chr","start","end") %in% names(getRegionObject$states))) stop("getRegionObject \"states\" component must contain chr, start, and end")
	if(!all(c("chr","start","end")%in%colnames(annotation))) stop("annotation must contain columns named chr, start, and end")
	...
	g2 = groupsplit[[2]]
	firstcolor = ifelse(1 %in% groupsplit[[1]],"blue","red")
	plot(xaxinds,counts[,1][xaxinds],type="l",lwd=0.5,col=firstcolor,ylim=c(-2,9),xaxt="n",
		xlim=c(plotlb,plotub),xlab="",ylab="Count")
	for(i in g1){
  		lines(xaxinds,counts[xaxinds,i],type="l",lwd=0.5,col="blue")
	}
	for(i in g2){
  		lines(xaxinds,counts[xaxinds,i],type="l",lwd=0.5,col="red")
	}
	mean1 = rowMeans(counts[xaxinds,g1])
	mean2 = rowMeans(counts[xaxinds,g2])
	lines(xaxinds,mean1,col="blue",lwd=3)
	lines(xaxinds,mean2,col="red",lwd=3)
...
plotGene <- function(getRegionObject, ind=NULL, genename=NULL, tstats, pos, annotation, counts, group, bppad = 50, axpad = 50, prettyskips = T, skiplines = T, countsheader=F, countssep="\t", tabname = NULL, plotfile = NULL, width = 900, height = 750, plottitle = NULL, chromosome,legendloc="bottomleft")
{
	if(!all(c("states.norle","states") %in% names(getRegionObject))) stop("getRegionObject must be a list with elements \"states.norle\" and \"states\" - usually the return of getRegions()")
	if(!all(c("chr","start","end") %in% names(getRegionObject$states))) stop("getRegionObject \"states\" component must contain chr, start, and end")
	if(!all(c("chr","start","end")%in%colnames(annotation))) stop("annotation must contain columns named chr, start, and end")
